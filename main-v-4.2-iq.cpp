...
void getCMBRLayerWCount2(int fid1, int fid2, int crow, bool cmbrFlag, bitset<FMAX> comb, int fCount) {
    ...
    for (int row = 0; row < GRID_ROWS; ++row)
    {
        for (int col = 0; col < GRID_COLS; ++col)
        {
                ...
                for (int i = 0; i < box1.size(); ++i)
                {
                    ...
                    for (int j = 0; j < mbr_array[n2row][n2col][fid2].size(); ++j)
                    {
                        ...
                        {  
                            ...
                            t2.push_back(j + instance_sum[fid2]); 
                            instance_array[row + iii/2][col + iii%2][init_layer_count][0].insert(j + instance_sum[fid2]);                           
                            cout << row + iii/2 << " " << col + iii%2 << " " << j + instance_sum[fid2] << endl;
                        } 
                    }
                    ...
                    if (t2.size() > 0)
                    {
                        ...
                        if (!cmbrFlag)
                        {
                            instance_array[row + iii/2][col + iii%2][init_layer_count][1].insert(i + instance_sum[fid1]);                
                        } 
                    }
                }
                ...
                {                    
                    ...
                    if (init_layer_count < s)
                    {
                        ...
                        if (cmbrFlag)
                        {
                            ...
                            ret.list1 = instanceCombinationBuild(l1, cmbr_map[n1row][n1col][crow][fid1].list1, cmbr_map[n1row][n1col][crow][fid1].list2, init_layer_count, row + iii/2, col + iii%2);                            
            ...
            // update cumulative instance count
            if (!cmbrFlag)
            {
                instance_sum[fid1] += mbr_array[row][col][fid1].size();              
            }
            instance_sum[fid2] += mbr_array[row][col][fid2].size();          
        }
    }
    instance_sum.clear();
}
