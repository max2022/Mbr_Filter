...
bool isCombinationValid(thirteenBits const c) {
    ...
    for (int i = 0; i < seletedFeaturesSizes.size(); ++i)
    {
        flag = true;
        input.reset();
        for (int j = 0; j < seletedFeaturesSizes[i]; ++j)
        {
            input[FMAX - 1 - seletedFeatures[st+j]] = 1;           
            if (c[FMAX - 1 - seletedFeatures[st+j]] ^ 1)
            {
                flag = false;
                break;
            }
...
int main()
{
    ...
    // thirteenBits tt(string("0000100000110"));
    // cout << isCombinationValid(tt);
