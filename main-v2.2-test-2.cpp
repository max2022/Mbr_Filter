...
vector<vector<vector<vector<cmbr_comb>>>> cmbr_map(FMAX-1);
...
void erase_cmbr_map(int row, int col, int k, vector<int> erase_list)
{
    //before erase
    ...
    for(int ii=erase_list.size()-1; ii >= 0 ; ii--)
    {
        ...
        cmbr_map[row][col][k].erase(cmbr_map[row][col][k].begin() + erase_list[ii]);
        cmbr_arr[row][col][k].erase(cmbr_arr[row][col][k].begin() + erase_list[ii]);
    }
    auto stop = high_resolution_clock::now();
    auto duration = duration_cast<microseconds>(stop - start); 
