...
#include "readFille_nongrid.cpp"

using namespace std::chrono;

// distance threshold
float const DIST = 5.0;
//float const DIST = 200.0; //20k data
// prevalence threshold
double const PI = 0.3;
//double const PI = 0.25; //20k data
int prev_size = 0;
...
void erase_cmbr_map(int k, vector<int> erase_list)
{
    ...
    for(int ii=erase_list.size()-1; ii >= 0 ; ii--)
    {
		...
		cmbr_arr[k][erase_list[ii]].isDeleted = true;
...
void cmbr_filter_layerwise(int k)
{
    ...
    {
	if(cmbr_map[k][ii].isDeleted == false){
		string result = "";
        double pr = 1.0;
	...
	{  
        ...
        if (k == 0) {
            ...
            //{   
				...
				{
				if(cmbr_arr[i][jj].isDeleted == false){
